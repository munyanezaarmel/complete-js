...
const deposit = function (movements) {
  const depositelement = movements.filter(mov => mov > 0);
};
deposit(account1.movements);
const withdrawal = function (movements) {
  const withdrawelement = movements.filter(mov => mov < 0);
};
withdrawal(account1.movements);
...
btnLogin.addEventListener('click', function (e) {
  ...
  calcDisplay(currentUser.movements);
